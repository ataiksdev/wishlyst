      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div className="flex flex-col gap-8">
            ...
            <h1 className="font-serif text-3xl sm:text-5xl lg:text-7xl tracking-tight text-foreground text-balance leading-[1.1]">
...
function WishlistItem({ name, price, tag, claimed }: { name: string; price: string; tag: string; claimed: boolean }) {
  return (
    <div className={`flex items-center justify-between rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 ${claimed ? "bg-secondary/60" : "bg-background"} border border-border`}>
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        <div className={`h-7 w-7 sm:h-8 sm:w-8 rounded-lg flex items-center justify-center text-[10px] sm:text-xs font-medium shrink-0 ${claimed ? "bg-accent/20 text-accent" : "bg-muted text-muted-foreground"}`}>
          {claimed ? (
        ...
        <div className="min-w-0">
          <p className={`text-xs sm:text-sm font-medium truncate ${claimed ? "line-through text-muted-foreground" : "text-foreground"}`}>{name}</p>
          <span className="text-[10px] sm:text-xs text-muted-foreground">{tag}</span>
        </div>
      </div>
      <span className={`text-[11px] sm:text-sm font-medium shrink-0 ml-2 ${claimed ? "text-muted-foreground" : "text-foreground"}`}>{price}</span>
    </div>
  )
}
